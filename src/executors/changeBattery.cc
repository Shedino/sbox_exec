...
void Exec::ChangeBattery::start () {
	...
	try {
		...
		ROS_INFO ("Exec::ChangeBattery: Execution unit: %s", tni.execution_ns.c_str());
		...
		ROS_INFO ("Exec::ChangeBattery: FINISHED");

		wait_for_postwork_conditions ();
	}
	catch (boost::thread_interrupted) {
		ROS_ERROR("BOOST INTERUPTED IN ChangeBattery");
