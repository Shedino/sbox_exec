...
Exec::ZeroRevolver::ZeroRevolver (std::string ns, int id) : Executor (ns, id) {
...
bool Exec::ZeroRevolver::prepare () {
  bool res = true;
  ROS_INFO ("Exec::ZeroRevolver::prepare");
...
void Exec::ZeroRevolver::start () {
	ROS_INFO ("Exec::ZeroRevolver::start: %s - %d", node_ns.c_str(), node_id);
	try {
		...
		ROS_INFO ("Exec::ZeroRevolver: Execution unit: %s", tni.execution_ns.c_str());
		...
		ROS_INFO ("Exec::ZeroRevolver: FINISHED");

		wait_for_postwork_conditions ();
	}
	catch (boost::thread_interrupted) {
		ROS_ERROR("BOOST INTERUPTED IN ZeroRevolver");
